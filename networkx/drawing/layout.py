...
def key_layout(G, keys=None):
    """Position nodes at the values of the keys input variable.

    For every node, a position is generated by setting the first dimension to
    G.node.items()[keys[0]] the second dimension to G.node.items()[keys[1] etc.
    ...
    keys : a list of hashable objects used to index into each node. The first
            element in keys is used for the first dimension and so on.
    ...
    --------
    >>> G = nx.Graph()
    >>> G.add_node(0, 'x'=1.2, 'y'=4)
    >>> G.add_node(1, 'x'=-0.2, 'y'=2.4)
    >>> pos = nx.key_layout(G, keys=['x', 'y'])

    """
    import numpy as np

    try:
        pos = [[d[k] for k in keys] for n,d in G.node.items()]
    except KeyError:
        msg = 'key %s not an attribue of node %d' % (k, n)
        raise ValueError(msg)
    
    # if the below type cast to 32 bit float fails on a value in pos, the 
    #   ValueError message is descriptive enough so we wont catch that one.
